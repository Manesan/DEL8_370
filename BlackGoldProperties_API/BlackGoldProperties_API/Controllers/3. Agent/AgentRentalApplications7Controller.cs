...
{
    public class AgentRentalApplications7Controller : ApiController
    {
        ...
        [HttpGet]
        [Route("api/agentrentalapplication7")]
        public IHttpActionResult Get([FromUri] string token)
        {
            ...
            if (TokenManager.GetRoles(token).Contains(5 /*Administrator*/) || TokenManager.GetRoles(token).Contains(1 /*Director*/) || TokenManager.GetRoles(token).Contains(2 /*Agent*/))
            {
                ...
                var agentrentalapplication7 = db.RENTALAPPLICATIONs.Where(z => z.RENTALAPPLICATIONSTATUSID == 7).Select(x => new {
                ...
                if (agentrentalapplication7 == null)
                {
                    return BadRequest();
                }
                else
                {
                    return Ok(agentrentalapplication7);
                }
        ...
        public IHttpActionResult Patch([FromUri] string token, [FromUri] int id, [FromUri] bool accepted, [FromUri] string note, [FromBody] DocumentController.UploadClass rentalagreement) //--Store link to unsigned agreement
        {
            ...
            if (TokenManager.GetRoles(token).Contains(5 /*Administrator*/) || TokenManager.GetRoles(token).Contains(1 /*Director*/) || TokenManager.GetRoles(token).Contains(2 /*Agent*/))
            {
                ...
                var db = LinkToDBController.db;
                db.Configuration.ProxyCreationEnabled = false;
                var agentrentalapplication7 = db.RENTALAPPLICATIONs.Include(x => x.TERM).Include(x => x.RENTAL).FirstOrDefault(x => x.RENTALAPPLICATIONID == id);
                ...
                {
                    if (agentrentalapplication7.RENTALID == null) //Only creates a rental record if this is a brand new rental application
                    {
                        agentrentalapplication7.RENTALAPPLICATIONSTATUSID = 2; //Sets the application status to 'Approved'
                        agentrentalapplication7.RENTALAPPLICATIONNOTE = note;

                        var property = agentrentalapplication7.PROPERTYID;
                        ...
                        {
                            PROPERTYID = agentrentalapplication7.PROPERTY.PROPERTYID,
                            RENTALSTATUSID = 4,//Sets to 'Pending client acceptance'
                            USERID = agentrentalapplication7.USERID,
                            RENTALDATESTART = agentrentalapplication7.RENTALAPPLICATIONSTARTDATE,
                            RENTALDATEEND = agentrentalapplication7.RENTALAPPLICATIONSTARTDATE.AddMonths(agentrentalapplication7.TERM.TERMDESCRIPTION),
                            TERMID = agentrentalapplication7.TERMID
                        });
                        ...
                        agentrentalapplication7.RENTALID = lastrentalid;

                    }
                    else //Called when the agent is accepting an extension/renewal     ----UPDATE RENTALTERM IN RENTAL ENTITY
                    {
                        agentrentalapplication7.RENTALAPPLICATIONSTATUSID = 7; //Sets the application status to 'Approved Extension'      --- use if statement to check status if its renewal or extension
                        agentrentalapplication7.RENTALAPPLICATIONNOTE = note;
                        agentrentalapplication7.RENTAL.RENTALSTATUSID = 7; //Sets the rental status to 'Pending Client Extension Acceptance'     -----THis doesnt work
                        ...
                        db.RENTALDOCUMENTs.Add(new RENTALDOCUMENT
                        {
                            RENTALID = agentrentalapplication7.RENTALID,
                            RENTALAGREEMENTDOCUMENT = documenturi
                        });
                    }
                }
                ...
                {
                    agentrentalapplication7.RENTALAPPLICATIONSTATUSID = 3; //Sets the application status to 'Rejected'
                    agentrentalapplication7.RENTALAPPLICATIONNOTE = note;  //---check if screen exists 
                }
