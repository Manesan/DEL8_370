...
{
    public class AgentRentalApplications5Controller : ApiController
    {
        ...
        [HttpGet]
        [Route("api/agentrentalapplication5")]
        public IHttpActionResult Get([FromUri] string token)
        {
            ...
            if (TokenManager.GetRoles(token).Contains(5 /*Administrator*/) || TokenManager.GetRoles(token).Contains(1 /*Director*/) || TokenManager.GetRoles(token).Contains(2 /*Agent*/))
            {
                ...
                var agentrentalapplication5 = db.RENTALAPPLICATIONs.Where(z => z.RENTALAPPLICATIONSTATUSID == 5).Select(x => new {
                ...
                if (agentrentalapplication5 == null)
                {
                    return BadRequest();
                }
                else
                {
                    return Ok(agentrentalapplication5);
                }
        ...
        public IHttpActionResult Get([FromUri] string token, [FromUri] int id)
        {
            ...
            if (TokenManager.GetRoles(token).Contains(5 /*Administrator*/) || TokenManager.GetRoles(token).Contains(1 /*Director*/) || TokenManager.GetRoles(token).Contains(2 /*Agent*/))
            {
                try
                {
                    ...
                    var agentrentalapplication5 = db.RENTALAPPLICATIONs.Where(z => z.RENTALAPPLICATIONID == id).Select(x => new
                    {
                        x.RENTALAPPLICATIONID,
                        x.PROPERTY.PROPERTYID,
                        x.PROPERTY.PROPERTYADDRESS,
        ...
        public IHttpActionResult Patch([FromUri] string token, [FromUri] int id, [FromUri] bool accepted, [FromUri] string note, [FromBody] DocumentController.UploadClass rentalagreement) //--Store link to unsigned agreement
        {
            ...
            if (TokenManager.GetRoles(token).Contains(5 /*Administrator*/) || TokenManager.GetRoles(token).Contains(1 /*Director*/) || TokenManager.GetRoles(token).Contains(2 /*Agent*/))
            {
                ...
                var db = LinkToDBController.db;
                db.Configuration.ProxyCreationEnabled = false;
                var agentrentalapplication5 = db.RENTALAPPLICATIONs.Include(x => x.TERM).Include(x => x.RENTAL).FirstOrDefault(x => x.RENTALAPPLICATIONID == id);
                ...
                {
                    if (agentrentalapplication5.RENTALID == null) //Only creates a rental record if this is a brand new rental application
                    {
                        agentrentalapplication5.RENTALAPPLICATIONSTATUSID = 2; //Sets the application status to 'Approved'
                        agentrentalapplication5.RENTALAPPLICATIONNOTE = note;

                        var property = agentrentalapplication5.PROPERTYID;
                        ...
                        {
                            PROPERTYID = agentrentalapplication5.PROPERTY.PROPERTYID,
                            RENTALSTATUSID = 4,//Sets to 'Pending client acceptance'
                            USERID = agentrentalapplication5.USERID,
                            RENTALDATESTART = agentrentalapplication5.RENTALAPPLICATIONSTARTDATE,
                            RENTALDATEEND = agentrentalapplication5.RENTALAPPLICATIONSTARTDATE.AddMonths(agentrentalapplication5.TERM.TERMDESCRIPTION),
                            TERMID = agentrentalapplication5.TERMID
                        });
                        ...
                        agentrentalapplication5.RENTALID = lastrentalid;

                    }
                    else //Called when the agent is accepting an extension/renewal     ----UPDATE RENTALTERM IN RENTAL ENTITY
                    {
                        agentrentalapplication5.RENTALAPPLICATIONSTATUSID = 7; //Sets the application status to 'Approved Extension'      --- use if statement to check status if its renewal or extension
                        agentrentalapplication5.RENTALAPPLICATIONNOTE = note;
                        agentrentalapplication5.RENTAL.RENTALSTATUSID = 7; //Sets the rental status to 'Pending Client Extension Acceptance'     -----THis doesnt work
                        ...
                        db.RENTALDOCUMENTs.Add(new RENTALDOCUMENT
                        {
                            RENTALID = agentrentalapplication5.RENTALID,
                            RENTALAGREEMENTDOCUMENT = documenturi
                        });
                    }
                }
                ...
                {
                    agentrentalapplication5.RENTALAPPLICATIONSTATUSID = 3; //Sets the application status to 'Rejected'
                    agentrentalapplication5.RENTALAPPLICATIONNOTE = note;  //---check if screen exists 
                }
