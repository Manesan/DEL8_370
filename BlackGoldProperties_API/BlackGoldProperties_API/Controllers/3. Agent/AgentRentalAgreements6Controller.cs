...
{
    public class AgentRentalAgreements6Controller : ApiController
    {
        //READ ALL DATA//
        [HttpGet]
        [Route("api/agentrentalagreement6")]
        ...
        { /*
            ...
            //if (TokenManager.GetRoles(token).Contains(1 /*Director*/) || TokenManager.GetRoles(token).Contains(2 /*Agent*/))
            {
                try
                {
                    ...
                    var agentrentalagreement6 = db.RENTALs.Where(z => z.RENTALSTATUSID == 6).Select(x => new {
                        x.RENTALID,
                        x.RENTALDATESTART,
                        x.RENTALDATEEND,
                    ...
                    if (agentrentalagreement6 == null)
                    {
                        return BadRequest();
                    }
                    else
                    {
                        return Ok(agentrentalagreement6);
                    }
                }
                catch (Exception)
                {
        ...
        { /*
            ...
            //if (TokenManager.GetRoles(token).Contains(1 /*Director*/) || TokenManager.GetRoles(token).Contains(2 /*Agent*/))
            {
                try
                {
                    ...
                    var agentrentalagreement6 = db.RENTALs.Where(z => z.RENTALID == id).Select(x => new {
                        x.RENTALID,
                        x.RENTALDATESTART,
                        x.RENTALDATEEND,
                        x.RENTALSTATU.RENTALSTATUSID,
        ...
        { /*
            ...
            //if (TokenManager.GetRoles(token).Contains(1 /*Director*/) || TokenManager.GetRoles(token).Contains(2 /*Agent*/))
            {
                try
                {
                    ...
                    //Find city
                    var agentrentalagreement6 = db.RENTALs.FirstOrDefault(x => x.RENTALID == id);
                    if (agentrentalagreement6 == null)
                        return NotFound();

                    //Delete specified city
                    agentrentalagreement6.RENTALSTATUSID = 1; // ---This sets the rental status to Available.. but maybe it should set it to something like Terminated as it is technically not available as yet??
