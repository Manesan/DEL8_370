    ...
    public class AgentRentalApplications8Controller : ApiController
    {
        ...
        public IHttpActionResult Patch([FromUri] string token, [FromUri] int id, [FromUri] bool accepted, [FromUri] string note, [FromBody] DocumentController.UploadClass rentalagreement) //--Store link to unsigned agreement
        {
            ...
            if (TokenManager.GetRoles(token).Contains(5 /*Administrator*/) || TokenManager.GetRoles(token).Contains(1 /*Director*/) || TokenManager.GetRoles(token).Contains(2 /*Agent*/))
            {
                ...
                var db = LinkToDBController.db;
                db.Configuration.ProxyCreationEnabled = false;
                var agentrentalapplication8 = db.RENTALAPPLICATIONs.Include(x => x.TERM).Include(x => x.RENTAL).FirstOrDefault(x => x.RENTALAPPLICATIONID == id);
                ...
                {
                    if (agentrentalapplication8.RENTALID == null) //Only creates a rental record if this is a brand new rental application
                    {
                        agentrentalapplication8.RENTALAPPLICATIONSTATUSID = 2; //Sets the application status to 'Approved'
                        agentrentalapplication8.RENTALAPPLICATIONNOTE = note;

                        var property = agentrentalapplication8.PROPERTYID;
                        ...
                        {
                            PROPERTYID = agentrentalapplication8.PROPERTY.PROPERTYID,
                            RENTALSTATUSID = 4,//Sets to 'Pending client acceptance'
                            USERID = agentrentalapplication8.USERID,
                            RENTALDATESTART = agentrentalapplication8.RENTALAPPLICATIONSTARTDATE,
                            RENTALDATEEND = agentrentalapplication8.RENTALAPPLICATIONSTARTDATE.AddMonths(agentrentalapplication8.TERM.TERMDESCRIPTION),
                            TERMID = agentrentalapplication8.TERMID
                        });
                        ...
                        agentrentalapplication8.RENTALID = lastrentalid;

                    }
                    else //Called when the agent is accepting an extension/renewal     ----UPDATE RENTALTERM IN RENTAL ENTITY
                    {
                        agentrentalapplication8.RENTALAPPLICATIONSTATUSID = 7; //Sets the application status to 'Approved Extension'      --- use if statement to check status if its renewal or extension
                        agentrentalapplication8.RENTALAPPLICATIONNOTE = note;
                        agentrentalapplication8.RENTAL.RENTALSTATUSID = 7; //Sets the rental status to 'Pending Client Extension Acceptance'     -----THis doesnt work
                        ...
                        db.RENTALDOCUMENTs.Add(new RENTALDOCUMENT
                        {
                            RENTALID = agentrentalapplication8.RENTALID,
                            RENTALAGREEMENTDOCUMENT = documenturi
                        });
                    }
                }
                ...
                {
                    agentrentalapplication8.RENTALAPPLICATIONSTATUSID = 3; //Sets the application status to 'Rejected'
                    agentrentalapplication8.RENTALAPPLICATIONNOTE = note;  //---check if screen exists 
                }
