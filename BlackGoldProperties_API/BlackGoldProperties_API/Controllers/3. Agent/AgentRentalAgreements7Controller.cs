...
{
    public class AgentRentalAgreements7Controller : ApiController
    {
        //READ ALL DATA//
        [HttpGet]
        [Route("api/agentrentalagreement7")]
        ...
        { /*
            ...
            //if (TokenManager.GetRoles(token).Contains(1 /*Director*/) || TokenManager.GetRoles(token).Contains(2 /*Agent*/))
            {
                try
                {
                    ...
                    var agentrentalagreement7 = db.RENTALs.Where(z => z.RENTALSTATUSID == 7).Select(x => new {
                        x.RENTALID,
                        x.RENTALDATESTART,
                        x.RENTALDATEEND,
                    ...
                    if (agentrentalagreement7 == null)
                    {
                        return BadRequest();
                    }
                    else
                    {
                        return Ok(agentrentalagreement7);
                    }
                }
                catch (Exception)
                {
        ...
        { /*
            ...
            //if (TokenManager.GetRoles(token).Contains(1 /*Director*/) || TokenManager.GetRoles(token).Contains(2 /*Agent*/))
            {
                try
                {
                    ...
                    var agentrentalagreement7 = db.RENTALs.Where(z => z.RENTALID == id).Select(x => new {
                        x.RENTALID,
                        x.RENTALDATESTART,
                        x.RENTALDATEEND,
                        x.RENTALSTATU.RENTALSTATUSID,
        ...
        { /*
            ...
            //if (TokenManager.GetRoles(token).Contains(1 /*Director*/) || TokenManager.GetRoles(token).Contains(2 /*Agent*/))
            {
                try
                {
                    ...
                    //Find city
                    var agentrentalagreement7 = db.RENTALs.FirstOrDefault(x => x.RENTALID == id);
                    if (agentrentalagreement7 == null)
                        return NotFound();

                    //Delete specified city
                    agentrentalagreement7.RENTALSTATUSID = 1; // ---This sets the rental status to Available.. but maybe it should set it to something like Terminated as it is technically not available as yet??
